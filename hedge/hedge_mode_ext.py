...
class HedgeBot:
    ...
    def __init__(
        ...
        self.pending_unwind_qty = Decimal("0")
        self.pending_unwind_side = None
    ...
    async def handle_extended_order_update(self, order_data):
        ...
        async with self.hedge_lock:
            ...
            if self.pending_unwind_qty > 0 and self.pending_unwind_side == side:
                unwind_offset = min(self.pending_unwind_qty, delta)
                self.hedged_pos = max(Decimal("0"), self.hedged_pos - unwind_offset)
                self.pending_unwind_qty -= unwind_offset
                if self.pending_unwind_qty == 0:
    ...
    async def maybe_unwind_hedged_pos(self, source: str):
        async with self.unwind_lock:
            ...
            self.pending_unwind_qty = unwind_qty
            self.pending_unwind_side = extended_side
            self.unwind_cooldown_until_ms = now_ms + self.unwind_cooldown_ms
            self.unwind_edge_bad_count = 0
